...
def loso_cross_validation(x, y, groups, message, num_classes):
    ...
    for i, (train_idx, test_idx) in enumerate(logo.split(x, y, groups=groups)):
        ...
        n_neighbors= 7,         # αριθμός γειτόνων
        weights='distance',    # μεγαλύτερο βάρος στους κοντινότερους
        metric='manhattan'
        )
...
loso_cross_validation(x, y_disease, groups, "k-NN - Predicting Disease",3)

# ➤ Πρόβλεψη Νόσου + Στάδιο (Disease_Level)
loso_cross_validation(x, y_disease_level, groups, "k-NN - Predicting Disease + Stage",5)
